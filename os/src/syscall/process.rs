...
pub fn sys_get_time(ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    let data_slice = unsafe {
        from_raw_parts(
            time_val.as_ref() as *const TimeVal as *const u8,
            size_of::<TimeVal>(),
        )
    };
...
pub fn sys_task_info(ti: *mut TaskInfo, times: &[u32]) -> isize {
    ...
    let mut task_info = Box::new(TaskInfo {
        status: TaskStatus::Running,
        syscall_times: [0; MAX_SYSCALL_NUM],
        time: get_time_ms(),
    });
    task_info.as_mut().syscall_times.copy_from_slice(times);

    let data_slice = unsafe {
        from_raw_parts(
            task_info.as_ref() as *const TaskInfo as *const u8,
            size_of::<TaskInfo>(),
        )
    };
...
pub fn sys_mmap(start: usize, len: usize, port: usize) -> isize {
    ...
    PROCESSOR.exclusive_access().map(
        start_va,
        end_va,
...
pub fn sys_set_priority(prio: isize) -> isize {
    ...
    if prio < 2 {
        ...
        PROCESSOR.exclusive_access().set_prio(prio as usize);
        prio
    }
}
